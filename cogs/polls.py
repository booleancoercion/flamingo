...
EMOJI_REGEX = re.compile(
    r"(<:\w{2,}:\d{,20}>|[\U00002702-\U000027B0\U0001F1E0-\U0001FAFF])")
    ...
    async def poll(self, ctx, emojis):
        ...
        if gid not in self.settings:
            raise commands.CommandError(
                "no polling channel set. Please set one using `fl!pollchannel`.")
        channel = msg.guild.get_channel(self.settings[gid]["poll_channel"])
        if channel == None:
            raise commands.CommandError(
                "invalid channel on record. Please set a new polling channel using `fl!pollchannel`.")
